...
object Example1 {
  def main (args: Array[String]): Unit = {
    ...
    //    def sum(number:Int,number1: Int,number2: Int,number3: Int,number4: Int)={       //求和，元素个数任意
        def sum(number:Int*):Int={       //求和，元素个数任意
          sum(1,2,3,4,6)
...
//        }
    //数组
        val arr1=new  Array[Int](5)    //新建数组包含5个元素
    arr1(3)=4//改变第3个元素
        ...
        for (i<-0 until(arr2.length,2)) println(arr1(i))  //间隔输出
        for (i<- (0 until arr2.length).reverse) println(arr1(i))
        arr2.sum  //逆序输出
    //    scala.util.Sorting.quickSort<arrBuffer>
        val arr3=for (i<-arr1 if (i%2==0)) yield  i*i
...
//        tuple._1                    //spark
    //移除一个数组中第一个负数后的所有负数
    val arrTomData=Array(1,34,-6,23,-1,24,-12,-23,23,45,-4,-34,-136,19)
    var bool,num,tol=0
    for (i <- 0 until(arrTomData.length,1)){
      if (bool==0 && arrTomData(i)< 0) bool=1
      else if (bool==1){
        if (arrTomData(i) < 0){
          num+=1
          tol+=1
        }else arrTomData(i-num)=arrTomData(i)
      }
    }
    val arr4=arrTomData.toBuffer
    arr4.remove(arrTomData.length-tol,tol)
    arr4.mkString(",")
  }
}
